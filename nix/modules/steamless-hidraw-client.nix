# NixOS integration for steamless-hidraw-client.
#
# The daemon itself is instantiated as a modular service
# (system.services.steamless-hidraw-client, imported from the package's
...
#    including Steam's).
#
# Capture follows the steamless-hidraw-capture.service lifecycle, so the
# controller can be handed back to local Steam at runtime:
#
#   systemctl stop steamless-hidraw-capture.service    # local Steam plays
#   systemctl start steamless-hidraw-capture.service   # remote passthrough
{
  config,
  lib,
  pkgs,
...
let
  ...
  captureFlag = "/run/steamless-hidraw/capture";

  matchHidrawScript = pkgs.writeShellScript "steamless-hidraw-match" ''
  ...
  captureCtl = pkgs.writeShellApplication {
    name = "steamless-hidraw-capturectl";
    runtimeInputs = [
      pkgs.coreutils
      pkgs.gnugrep
      pkgs.systemdMinimal
    ...
    text = ''
      ...
      case "''${1:-}" in
        ...
        stop)
          ...
          ;;
        *)
          echo "usage: steamless-hidraw-capturectl start|stop" >&2
          exit 64
          ;;
      esac
    '';
  ...
  captureUdevRules = pkgs.writeTextFile {
    name = "steamless-hidraw-udev-rules";
    destination = "/lib/udev/rules.d/72-steamless-hidraw-capture.rules";
    text = ''
...
in
{
  options.services.steamless-hidraw-client = {
    enable = lib.mkEnableOption "SteamlessLink hidraw-to-network controller bridge";
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.steamless-hidraw-capture = lib.mkIf cfg.capture.enable {
      description = "SteamlessLink controller capture (steal hidraw from local Steam)";
      ...
      partOf = [ "steamless-hidraw-client.service" ];
      serviceConfig = {
        Type = "oneshot";
    ...
    systemd.services.steamless-hidraw-client = lib.mkIf cfg.capture.enable {
      wants = [ "steamless-hidraw-capture.service" ];
      after = [ "steamless-hidraw-capture.service" ];
    };
  };
}
